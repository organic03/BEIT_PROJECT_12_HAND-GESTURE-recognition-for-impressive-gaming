...
#include<iostream>
#include<C:\opencv\build\include\opencv2\highgui\highgui.hpp>//KHUDKA PATH DAAL JIDAR YEH FILE HAI
#include<C:\opencv\build\include\opencv2\imgproc\imgproc.hpp>//KHUDKA PATH DAAL JIDAR YEH FILE HAI
#include<C:\opencv\build\include\opencv2\core\core.hpp>//KHUDKA PATH DAAL JIDAR YEH FILE HAI
#include<C:\opencv\build\include\opencv2\video\background_segm.hpp>//KHUDKA PATH DAAL JIDAR YEH FILE HAI
#include<Windows.h>
using namespace cv;
using namespace std;
...
int anglebetween(Point p1, Point p2)
{
	...
	if (p2.x == p1.x)
		return 0;
	ang = double(atan((double)(p1.y - p2.y)/(double)(p2.x - p1.x)) * 180/ 3.14);
	if (ang < 0)
		return ang + 180;
	else
		return ang;
